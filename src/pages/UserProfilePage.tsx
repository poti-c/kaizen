...
import { ArrowLeft, User, Mail, Building2, Shield, FolderOpen, Clock, CheckCircle2, AlertTriangle, TrendingUp, CalendarDays } from 'lucide-react'
...
import { getInitials, formatDateTime, formatDuration, isSLABreached } from '@/lib/utils'
...
export function UserProfilePage() {
  ...
  if (!user) return (
    <div className="p-6 text-center text-gray-400">{t.caseDetail.notFound}</div>
  )

  // Fine-grained access control (after user is loaded):
  // Super Admins: can view manager + staff profiles only (not other super admins)
  if (myProfile?.role === 'super_admin' && user.role === 'super_admin') {
    return <Navigate to="/users" replace />
  }
  // Managers: HR manager can view all managers + staff; others can view own + dept staff only
  if (myProfile?.role === 'manager') {
    const isHR = myProfile.department === 'human_resource'
    const canView = userId === myProfile.id ||
      (isHR
        ? user.role === 'manager' || user.role === 'staff'           // HR: all managers + staff
        : user.role === 'staff' && user.department === myProfile.department) // others: own dept staff
    if (!canView) return <Navigate to="/users" replace />
  }
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto animate-fade-in space-y-5">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {!user.is_active && (
              <span className="text-xs text-red-500 font-medium">{t.users.inactive}</span>
            )}
          </div>
        </div>
      ...
      {totalCases > 0 && (() => {
        ...
        const criteria = [
          {
            label: lang === 'th' ? 'อัตราการแก้ไข' : 'Resolution Rate',
            value: resolutionRate, color: '#22c55e',
            note: `${closedCases.length} / ${totalCases} ${lang === 'th' ? 'เคส' : 'cases'}`
          },
          {
            label: lang === 'th' ? 'ตรงตามกำหนดเวลา' : 'On-Time Delivery',
            value: onTimeScore, color: '#3b82f6',
            note: `${totalCases - overdueCases.length} / ${totalCases} ${lang === 'th' ? 'ตรงเวลา' : 'on time'}`
          },
          {
            label: lang === 'th' ? 'ความมีส่วนร่วม' : 'Engagement',
            value: activityScore, color: '#8b5cf6',
            note: `${activity.length} ${lang === 'th' ? 'กิจกรรม' : 'actions'}`
          },
        ]
            ...
            <div className="flex items-center gap-3 mb-4">
              <div className="flex items-center gap-2 flex-1">
                ...
                <h3 className="font-semibold text-sm text-gray-900">
                  {lang === 'th' ? 'คะแนนประสิทธิภาพโดยรวม' : 'Overall Performance Score'}
                </h3>
              </div>
              {/* Score badge */}
              <span className={`px-3 py-1 rounded-full text-sm font-bold border ${tier.bg} ${tier.text} ${tier.border}`}>
                {tier.label}
              </span>
            </div>
              ...
              <div className="relative flex-shrink-0 w-16 h-16">
                <svg className="w-16 h-16 -rotate-90" viewBox="0 0 56 56">
                  ...
                  <circle
                    ...
                    strokeDasharray={`${(weighted / 100) * 138.2} 138.2`}
                    strokeLinecap="round"
                  />
                </svg>
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-lg font-bold text-gray-900">{weighted}</span>
                </div>
              </div>
          ...
          <ResponsiveContainer width="100%" height={120}>
            <BarChart data={monthlyData} margin={{ top: 0, right: 0, left: -28, bottom: 0 }}>
              ...
              <Bar dataKey="count" radius={[3, 3, 0, 0]} maxBarSize={32}>
                {monthlyData.map((_, i) => (
                  <Cell key={i} fill="var(--brand-primary)" fillOpacity={i === 5 ? 1 : 0.45} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
            ...
            {activity.map(a => (
              <div key={a.id} className="flex items-start gap-3 px-5 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {a.case_number && (
                    <Link to={`/cases/${a.case_id}`} className="text-xs text-[var(--brand-primary)] hover:underline">
