...
interface OrderRow {
  ...
  fulfill_department: string | null
}

// Dashboard summary for the Routine Roster — mirrors PMSummaryCard. The hero
// "Fulfillment" is an on-time gauge: today's orders that aren't late ÷ today's
// total (so it starts at 100% and only dips when an order passes its ready-by).
export function RRSummaryCard() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (cancelled) return
      let list = (data as OrderRow[]) ?? []
      // Staff see only orders touching their department (requesting or fulfilling).
      if (isStaff && profile?.department) {
        list = list.filter(o => o.request_department === profile.department || o.fulfill_department === profile.department)
      }
      setRows(list)
      setLoading(false)
    })()
  ...
  const todays = rows.filter(o => o.order_date === today)
  const tomorrows = rows.filter(o => o.order_date === tomorrow)
  ...
  const forTomorrow = tomorrows.length
  ...
  const L = {
    ...
    fulfillment: lang === 'th' ? 'การจัดส่งตามเวลา' : 'Fulfillment',
    ...
    none: lang === 'th' ? 'ไม่มีออเดอร์ประจำสำหรับวันนี้หรือพรุ่งนี้' : 'No routine orders today or tomorrow.',
  }
