...
export function PMSettings() {
  ...
  async function save() {
    ...
    setBusy(false)
    if (error) toast.error(error.message); else toast.success('Maintenance settings saved')
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h2 className="font-semibold text-gray-900">Preventive Maintenance — Settings</h2>
      </div>
      <p className="text-xs text-gray-500 mb-4">Approval, reminder and escalation rules for maintenance tasks.</p>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          <label className="flex items-start gap-2.5 cursor-pointer">
            ...
            <span>
              <span className="text-sm font-medium text-gray-800">Require approval for maintenance tasks</span>
              <span className="block text-xs text-gray-500">When on, a completed task waits for the responsible department manager (or Top Management) to approve before the cycle advances.</span>
            </span>
          </label>
          ...
          <label className="flex items-start gap-2.5 cursor-pointer">
            ...
            <span>
              <span className="text-sm font-medium text-gray-800">Auto-escalate overdue maintenance to a Case</span>
              <span className="block text-xs text-gray-500">Creates a normal Case (priority High) so an overdue task can't be forgotten.</span>
            </span>
          </label>
          ...
          <button onClick={save} disabled={busy} className="flex items-center gap-1.5 bg-[var(--brand-primary)] text-white text-sm font-medium px-4 h-9 rounded-lg disabled:opacity-50">
            {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}Save settings
          </button>
        </div>
      )}
    </div>
